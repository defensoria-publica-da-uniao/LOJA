...
<h1 class="page-title">
    Unidades <small>Listagem registradas no sistema</small>
</h1>
<div class="page-bar">
    <ul class="page-breadcrumb">
        ...
        <li>
            ...
            <a href="<?php echo RAIZ . "unidades/listarUnidade"; ?>">Unidades</a>
        </li>
    </ul>
                ...
                <table class="table table-striped table-bordered table-hover dt-responsive" width="100%" id="sample_5">
                    <thead>
                        <tr>
                            ...
                            <th class="min-phone-l min-tablet desktop">Estado</th>
                            <th class="all">Unidade</th>
                            <th class="min-tablet desktop">E-mail</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php 
                            $resultUnidade = $oUnidade->listar();
                        ...
                        <tr>
                            <td>
                                <div class="btn-toolbar" style="margin-left:0px !important;">
                                    <div class="btn-group">
                                        <button type="button" class="btn btn-xs btn-default blue-madison mod popovers" data-toggle="modal" data-doc="<?php echo $arUnidade['id_unidade']; ?>" data-target='#editarCadastro' data-container="body" data-trigger="hover" data-placement="top" data-content="" data-original-title="Editar">
                                            <i class="fa fa-edit"></i>
                                        </button>
                                    </div>
                                    <div style="float: right !important;" > <!-- class="btn-group"  -->
                                        <?php 
                                            if( $arUnidade["str_situacao"] == 'D' ){ #Desativado
                                        ...
                                        ?>
                                        <form action="<?php echo CONTROLLER . 'unidade.php'; ?>" method="POST">
                                            <button type="submit" class="<?php echo $corBtn; ?> btn-xs mod" data-toggle="confirmation" data-original-title="<?php echo $msgAcao; ?>">
                                                <input type='hidden' name='arrDadosForm[str_situacao]' value="<?php echo $arUnidade["str_situacao"]; ?>" />
                                                <input type='hidden' name='arrDadosForm[id]' value="<?php echo $arUnidade['id_unidade']; ?>" />
                                                <input type="hidden" name="arrDadosForm[tabela]" value="tb_unidade" />
                                                <input type="hidden" name="arrDadosForm[campo_where]" value="id_unidade" />
                            ...
                            <td><?php echo utf8_encode($arUnidade['str_uf']); ?></td>
                            <td><?php echo utf8_encode($arUnidade['str_descricao']); ?></td>
                            <td><?php echo utf8_encode($arUnidade['str_email']); ?></td>
                        </tr>
                        <?php 
                            }
                        ?>
...
<?php
    include 'modal/editarUnidade.php';
    include 'modal/novaUnidade.php';
?>

<!-- Ajax para editar Unidade DPU -->
<script>
    $(document).ready(function () {
        $('#editarCadastro').on('show.bs.modal', function (e) {
           var idUnidade = $(e.relatedTarget).data('doc');
           $.ajax({
               type: 'POST',
               data: 'idUnidade='+idUnidade+'&method=listar&acao=ajax',
               url: '<?php echo CONTROLLER; ?>unidade.php', 
               success: function(data){
                    var response = $.parseJSON(data); 
                    $("#id").val(response.id_unidade);
                    $("#unidade").val(response.descricao);
